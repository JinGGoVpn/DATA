...
SERVER_PORT="$(cat ~/log-install.txt | grep -w "WIREGUARD" | cut -d: -f2|sed 's/ //g')"

SERVER_PUB_IP=$domain;
	...
	CLIENT_DNS_1=($(cat /etc/resolvconf/resolv.conf.d/head | awk '{print $2}'))

	CLIENT_DNS_2=($(cat /etc/resolvconf/resolv.conf.d/head | awk '{print $2}'))
